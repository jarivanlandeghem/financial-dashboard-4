    ...
    <div>
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">Your recurring monthly costs</p>
        </div>
      ...
      <div className="grid-3" style={{ marginBottom: 20 }}>
        <div className="stat-card">
          ...
          <div className="stat-change neutral">{active.length} active subscriptions</div>
        </div>
        <div className="stat-card">
        ...
        <div className="stat-card">
          ...
          <div className="stat-value" style={{ fontSize: 22, color: savingsIfCancel > 0 ? 'var(--green)' : 'var(--text-muted)' }}>
            {fmt(savingsIfCancel)}<span style={{ fontSize: 14 }}>/mo</span>
          </div>
          <div className="stat-change positive">{savingsIfCancel > 0 ? `€${(savingsIfCancel*12).toFixed(2)}/year if cancelled` : 'No cancellations marked'}</div>
        </div>
      </div>
      ...
      <div className="card" style={{ padding: 0, marginBottom: 20 }}>
        ...
        {active.map(sub => (
          <div key={sub.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '14px 20px', borderBottom: '1px solid var(--border)' }}>
            <div className="cat-icon" style={{ background: 'var(--accent-light)', fontSize: 20 }}>{sub.icon}</div>
            <div style={{ flex: 1 }}>
            ...
            <div style={{ fontWeight: 700, fontSize: 15, marginRight: 12 }}>{fmt(sub.amount)}<span style={{ fontSize: 11, fontWeight: 400, color: 'var(--text-muted)' }}>/mo</span></div>
            {sub.canCancel ? (
      ...
      {markedCancel.length > 0 && (
        <div className="card" style={{ padding: 0, border: '1px solid var(--green-light)' }}>
          ...
          {markedCancel.map(sub => (
            <div key={sub.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '14px 20px', borderBottom: '1px solid var(--border)', opacity: 0.7 }}>
              <div className="cat-icon" style={{ background: 'var(--green-light)', fontSize: 20 }}>{sub.icon}</div>
              <div style={{ flex: 1, textDecoration: 'line-through' }}>
                <div style={{ fontWeight: 600, fontSize: 14 }}>{sub.name}</div>
                <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>Will save {fmt(sub.amount * 12)}/year</div>
              </div>
          ...
          <div style={{ padding: '16px 20px', background: 'var(--green-light)', borderRadius: '0 0 var(--radius) var(--radius)' }}>
            <div style={{ color: 'var(--green)', fontWeight: 600, fontSize: 14 }}>
              💰 Cancelling these saves you {fmt(savingsIfCancel)}/month · {fmt(savingsIfCancel * 12)}/year
            </div>
          </div>
        </div>
      )}
