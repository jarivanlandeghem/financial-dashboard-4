...
export function applyAccentColor(hex, isDark) {
  const { r, g, b } = hexToRgb(hex);
  const { h, s, l } = rgbToHsl(r, g, b);
  const root = document.documentElement;

  if (isDark) {
    const light = hslToRgb(h, s, Math.min(l + 15, 78));
    const mid   = hslToRgb(h, s, Math.min(l + 25, 85));
    const lHex  = rgbToHex(light.r, light.g, light.b);
    root.style.setProperty('--accent',       lHex);
